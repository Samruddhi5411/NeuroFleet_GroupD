...
const VehicleCard = ({ vehicle, onEdit, onDelete, isCompact }) => {
  ...
  if (isCompact) {
    return (
      <div
        className={`glass-card p-4 hover:scale-105 transition-all duration-300 ${animateChange ? 'ring-2 ring-accent-cyan animate-pulse' : ''
          }`}
      >
  ...
  return (
    <div
      className={`glass-card p-6 hover:scale-[1.02] transition-all duration-300 ${animateChange ? 'ring-2 ring-accent-cyan shadow-lg shadow-accent-cyan/30' : ''
        }`}
    >
